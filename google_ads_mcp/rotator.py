"""
Фоновый ротатор заголовков и описаний для App Ads.
Запускается как отдельный процесс через start_rotation MCP tool.

Логика пула:
- Хранит все когда-либо использованные заголовки/описания
- Берёт только те, которые ещё НЕ использовались
- Когда пул исчерпан — генерирует 50 новых через Claude API
"""
import json
import sys
import time
...
def _setup_logging(campaign_id: str):
    log_file = f"/var/log/google-ads-rotator-{campaign_id}.log"
    # fallback to /tmp if /var/log not writable
    if not os.access("/var/log", os.W_OK):
        log_file = f"/tmp/google-ads-rotator-{campaign_id}.log"
    logging.basicConfig(
        level=logging.INFO,
...
# ── Стартовый пул (используется только при первом запуске) ─────────────────
...
def _state_file(campaign_id: str) -> str:
    return os.path.join(_STATE_DIR, f"state_{campaign_id}.json")
...
def _generate_new_texts(used_headlines: list, used_descriptions: list) -> tuple[list, list]:
    """Генерирует 50 новых заголовков и 30 описаний через OpenRouter API."""
...
Generate exactly:
- 50 headlines (max 30 characters each, no punctuation at end)
...
{{
  ...
  "descriptions": ["description1", "description2", ...]
    ...
    log.info("Генерирую новые тексты через OpenRouter API...")
...
# ── Выборка текстов с полным циклом ───────────────────────────────────────

def _pick_texts(campaign_id: str, n_headlines: int = 5, n_descriptions: int = 5) -> tuple[list, list]:
    state = _load_state(campaign_id)

    if len(state["available_headlines"]) < n_headlines:
        log.info(f"Заголовков осталось {len(state['available_headlines'])} — генерирую новые через Claude")
    ...
    if len(state["available_descriptions"]) < n_descriptions:
        log.info(f"Описаний осталось {len(state['available_descriptions'])} — генерирую новые через Claude")
        new_h, new_d = _generate_new_texts(state["used_headlines"], state["used_descriptions"])
        state["available_headlines"] = new_h
        state["available_descriptions"] = new_d
        state["used_headlines"] = []
    ...
    headlines = random.sample(state["available_headlines"], n_headlines)
    descriptions = random.sample(state["available_descriptions"], n_descriptions)
...
def run(campaign_id: str, customer_id: str = None, min_minutes: int = 10, max_minutes: int = 13):
    _setup_logging(campaign_id)
    cid_info = f", customer: {customer_id}" if customer_id else ""
    log.info(f"Ротатор запущен. Кампания: {campaign_id}{cid_info}, интервал: {min_minutes}-{max_minutes} мин")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m google_ads_mcp.rotator <campaign_id> [customer_id]")
        sys.exit(1)
